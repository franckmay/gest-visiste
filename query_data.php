...
.affix {
  ...
  <div class="container-fluid">
    <div class="navbar-header ">
     <a class="navbar-brand" href="#" id = "li"><?php echo "Connecte comme : ".$user2;?></a>
    </div>
    <ul class="nav navbar-nav navbar-right">
      <li><a href="front.php" id = "li">Accueil</a></li>
      <li ><a href="myform.php" id = "li">Ajouter Visiteur</a></li>
      <li><a href="logoutform.php" id = "li">Verifier visiteur</a></li>
      <li class="active"><a href="query_data.php">Afficher les données</a></li> 
      <li><a href="logout.php" id = "li">Se déconnecter</a></li> 
    </ul>
  </div>
</nav>
	...
	<div id='body'>
		...
		<div class="FormLabel" style="border-top: solid 2px;">		
		 <label for = "all" >Voir toutes les entrées :</label><input type = "radio" id = "allData" name = "search"  value = "all" onclick="disable()">
		 </div>
		 <div class="FormLabel" style="border-top: solid 2px;">
		 <label for =" active entries">Voir tous les visiteurs actifs : </label><input type = "radio" name = "search"  id="active" value ="active" onclick="disable()">
		 </div>
		 <div style="border-bottom: solid 2px;padding-bottom: 12px;">
		 <div class="FormLabel" style="border-top: solid 2px;">
		 <label for = "date">Recherche par date :</label> <input type = "radio" id = "bydate" name = "search" value="dates" 
		 onclick="undisable()">
		</div>
    ...
    <div>
          ...
          <input type="date" id="dateP" name = "dateP"  id = "in1" oninput = "onDateInput()" disabled required /> 
          <input type="date" id="dateF" name = "dateF"  id = "in2" disabled required />
...
var inputDateA = document.getElementById('in1').value;
  ...
  <div id = "button">
   <button type= "submit" name = "submit"  class="btn btn-success"><span class="glyphicon glyphicon-search"></span>&nbsp;Rechercher</button>
  </div>
  </form>
  </div>
...
<?php
 ...
 if(isset($_POST["search"]) && $_POST["search"]=="all") 
 {
	...
	if($count){
		echo "<br><h3 style = 'padding-left: 16px;''>Information de tous les visiteurs :</h3><br/>";
		headingMake($result);
	}else{echo "<br><span style = 'color : red;'>Aucune entrée à afficher</span>";}
 	...
 	if( empty($_POST["day_start"]) && empty($_POST["day_end"]) && empty($_POST["active"])) {
 	 ...
 	 $count = mysqli_num_rows($result);
 		...
 		if($count){
 			...
 			echo "<br><h3 style = 'padding-left: 16px;''>Informations sur les visiteurs pour les visiteurs actifs :</h3><br/>";
 			headingMake($result);
